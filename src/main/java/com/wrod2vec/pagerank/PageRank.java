...
public class PageRank {
    ...
    public List<PageRankResultDTO> getRankedResults() {

        List<PageRankResultDTO> nodes = new ArrayList<>();
        for (PageRankNode node : rankedList) {
            PageRankResultDTO item = new PageRankResultDTO();
            item.setNode(node.getIdentifier());
            item.setValue(node.getRank());
            nodes.add(item);
        }
            return nodes;
        }
        ...
        public static void main (String[]args){
             ...
             */
            //String readAddr =  "testFile/tfacts_result.txt";

            String readAddr = "testFile/cache/pagerankInputCachestartI=1&stopi=1&startj=1&endj200.txt";
            String writeAddr = "testFile/rankedResults.txt";
            int iterations = 10;
